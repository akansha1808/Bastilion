...
public class SystemDB {
    ...
    public static SortedSet getUserSystemSet(SortedSet sortedSet, Long userId) throws SQLException, GeneralSecurityException {
        ...
        String sql = "select * from systems where id in (select distinct system_id from  system_map m, user_map um where m.profile_id=um.profile_id and um.user_id=? ";
    ...
    public static SortedSet getSystemSet(SortedSet sortedSet, Long profileId) throws SQLException, GeneralSecurityException {
        ...
        String sql = "select s.*, m.profile_id from  systems s left join system_map  m on m.system_id = s.id and m.profile_id = ? " + orderBy;
    ...
    public static SortedSet getSystemSet(SortedSet sortedSet) throws SQLException, GeneralSecurityException {
        ...
        String sql = "select * from  systems s ";
        //if profile id exists add to statement
    ...
    public static HostSystem getSystem(Connection con, Long id) throws SQLException {
        ...
        PreparedStatement stmt = con.prepareStatement("select * from  systems where id=?");
        stmt.setLong(1, id);
        ResultSet rs = stmt.executeQuery();
    ...
    public static Long insertSystem(HostSystem hostSystem) throws SQLException, GeneralSecurityException {
        ...
        Long userId = null;
        Connection con = DBUtils.getConn();
        PreparedStatement stmt = con.prepareStatement("insert into systems (display_nm, username, host, port, authorized_keys, status_cd) values (?,?,?,?,?,?)", PreparedStatement.RETURN_GENERATED_KEYS);
    ...
    public static void updateSystem(HostSystem hostSystem) throws SQLException, GeneralSecurityException {
        ...
        PreparedStatement stmt = con.prepareStatement("update systems set display_nm=?, username=?, host=?, port=?, authorized_keys=?, status_cd=?  where id=?");
        stmt.setString(1, hostSystem.getDisplayNm());
        stmt.setString(2, hostSystem.getUser());
        stmt.setString(3, hostSystem.getHost());
        stmt.setInt(4, hostSystem.getPort());
    ...
    public static void deleteSystem(Long hostSystemId) throws SQLException, GeneralSecurityException {
        ...
        PreparedStatement stmt = con.prepareStatement("delete from systems where id=?");
    ...
    public static List<HostSystem> getAllSystems() throws SQLException, GeneralSecurityException {
        ...
        Connection con = DBUtils.getConn();
        PreparedStatement stmt = con.prepareStatement("select * from systems");
        ResultSet rs = stmt.executeQuery();
    ...
    public static List<Long> getAllSystemIdsForUser(Connection con, Long userId) throws SQLException {
        ...
        PreparedStatement stmt = con.prepareStatement("select distinct system_id from system_map m, user_map um, systems s where m.profile_id=um.profile_id and um.user_id=?");
        stmt.setLong(1, userId);
        ResultSet rs = stmt.executeQuery();
    ...
    public static void saveSystem(String displayName, String hostname, String username, String protocol, int port) 
            throws SQLException, GeneralSecurityException {
        Connection con = null;
        try {
            con = DBUtils.getConn();
            PreparedStatement stmt = con.prepareStatement(
                "INSERT INTO systems (display_nm, host, username, port, status_cd) VALUES (?,?,?,?,?)");
